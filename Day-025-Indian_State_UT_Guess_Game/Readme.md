...
* Lastly, I learned how to save my own filtered data into a new .csv file. First, I take my list (or dictionary), convert it into a DataFrame using pandas.DataFrame(), and then save it using .to_csv("states_to_learn.csv", index=False). The index=False part is important—it just makes sure row numbers don’t get saved in the file.
...
* The CSV file (`india_states_ut_centers.csv`) contains a list of all Indian states/UTs and their `x` and `y` coordinates.
