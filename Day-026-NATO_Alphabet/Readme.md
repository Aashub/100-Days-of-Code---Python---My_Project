...
* How to loop in Dataframe : In order to looping in dataframe we use .iterrows() to go through each row one at a time. It gives you the index and also the row itself. From there, you can use things like row.letter and row.code to get the actual values of the dataframe.
...
* Then using list comprehension the program loops through each character in the input and checks if that character is in the dictionary.
